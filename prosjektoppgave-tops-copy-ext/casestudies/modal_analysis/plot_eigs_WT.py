...
if __name__ == '__main__':
    ...
    print(' ')
    print('Speed states: ')
    speedstates_WT = ps.windturbine['WindTurbine'].state_idx_global['omega_m']
    print(speedstates_WT)
    ...
    for tellerx in speedstates_WT:
        if abs(rev[tellerx, critmode]) > maxmode:
            maxmode = abs(rev[tellerx, critmode])
            tellermax=tellerx
            Gennumber= int((tellerx-speedstates_WT[0]+6)/6)
